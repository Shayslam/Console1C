...
#Область ОбработчикиСобытийЭлементовТаблицыФайлыДерево
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРасширенияТаблицы
...
Процедура СППоискПоСправке(Команда)
	...
	Если Элементы.СтраницыПравые.Видимость = Ложь
		Или Элементы.СтраницаСинтаксПомощник.Видимость = Ложь Тогда
...
Процедура ВыполнитьКод1С(Конфигурация, Текст)
	...
	Если Конфигурация.Контекст = "НаКлиенте" Тогда
		ВыполнитьКод(Текст, Ложь);
	Иначе
		ВыполнитьКодНаСервере(Текст, Ложь);
	КонецЕсли;
...
&НаСервере
Процедура ВыполнитьЗапрос(Текст)
...
Процедура ВыполнитьКодЧерезОбработку(Конфигурация, Текст, ТекстМодуляОбъекта)
	...
	Если (Конфигурация.Свойство("Контекст") И Конфигурация.Контекст = "НаСервере")
		Или Не Конфигурация.Свойство("Файл") Тогда
		ВыполнитьКодЧерезОбработкуНаСервере(ИмяОбработки, МетодВхода, СтрокаПараметров);
...
Процедура ВыполнитьКодЧерезОбработкуНаСервере(ИмяОбработки, МетодВхода, СтрокаПараметров)
...
Процедура СкопироватьФайлы(Знач КаталогИсточник, Знач КаталогПриемник)
   ...
   Исключение
	   ...
	   ВызватьИсключение "Ошибка при копировании: " + ОписаниеОшибки()
...
&НаКлиенте
Процедура ЗаполнитьШаблонОбработки(ИмяКаталогаГотовойОбработки, Конфигурация, ТекстМодуляОбработки = "",
	ТекстМодуляФормы = "")
	...
	ИмяМодуляФормы = ИмяКаталогаГотовойОбработки + "Form" + РазделительПути + ИмяФормыОбработки
		+ РазделительПути + "Form.obj.bsl";
...
Функция ВыполнитьКодЧерезОбработкуПроверкаКонфигурации(Конфигурация, МетодВхода)
	...
	Если Не Конфигурация.Свойство("МетодВхода") И 
		(Не Конфигурация.Свойство("ОткрытиеОбработки") Или Конфигурация.ОткрытиеОбработки = Ложь) Тогда
...
Функция ПолучитьТекстДляОбработки(Текст = Неопределено, ИспользоватьСтарыйАдрес = Ложь)
	...
	|_SPd201bf6 = ПолучитьИзВременногоХранилища(""" + АдресПеременных + """);
	|
	|Попытка
	|	_SPd201bf6.Добавить(ПоместитьВоВременноеХранилище(SPd201bf6, Новый УникальныйИдентификатор));
	|	ПоместитьВоВременноеХранилище(_SPd201bf6, """ + АдресПеременных + """);
...
Процедура ВыполнитьКодЧерезОбработкуСРасчетомПеременных(ИмяОбработки, МетодВхода = Неопределено, СтрокаПараметров = "")
	...
	Форма = ОткрытьФорму("ВнешняяОбработка." + ИмяОбработки + ".Форма", , ЭтаФорма, Истина, ТекущееОкно, , ,
...
Функция СправкаОбОбработке()
	...
	ТекстСведений = "Наименование: " + ОбОбработке.Наименование + " ("
		+ ОбОбработке.Версия + ")"
		+ Символы.ПС + "Вид: " + ОбОбработке.Вид
		+ Символы.ПС + "Назначение: " + ?(ЗначениеЗаполнено(ОбОбработке.Назначение), ОбОбработке.Назначение, "Нет")
		+ Символы.ПС + "Безопасный режим: " + ОбОбработке.БезопасныйРежим
		+ Символы.ПС + "Информация: " + ОбОбработке.Информация
		+ Символы.ПС + Символы.ПС + "Команда:"
		+ Символы.ПС + " 1. Представление: " + ОбОбработке.Команды[0].Представление + ";"
		+ Символы.ПС + " 2. Идентификатор: " + ОбОбработке.Команды[0].Идентификатор + ";"
		+ Символы.ПС + " 3. Использование: " + ОбОбработке.Команды[0].Использование + ";"
		+ Символы.ПС + " 4. Показывать оповещения: " + ОбОбработке.Команды[0].ПоказыватьОповещение + "."
		+ Символы.ПС + " 5. Модификатор: "
...
Процедура ФайлыДеревоОбновлениеИмениФайлаРекурсивно(ИзмененнаяСтрока, ПроверяемаяСтрока, СтароеИмяКаталога)
	...
	Если Не ПустаяСтрока(СтароеИмяКаталога) Тогда
		Файл = Новый Файл(СтрЗаменить(ПроверяемаяСтрока.ПолноеИмя + РазделительПути, СтароеИмяКаталога + РазделительПути,
...
Функция ФайлыТекстЗапускаПоУмолчанию()
	...
	Возврат
	"// Конфигураций может быть любое количество, имена конфигураций должны быть разными
	...
	Для Каждого Путь Из МассивПутей Цикл
		...
		Если Файл.Существует() Тогда
			...
			Если НРег(Файл.Расширение) = ".erf" Тогда
				ИмяРасширения = ПодключитьОтчет(Путь, ПоместитьВоВременноеХранилище(РасширениеДД, УникальныйИдентификатор));
			Иначе
				ИмяРасширения = ПодключитьОбработку(Путь, ПоместитьВоВременноеХранилище(РасширениеДД, УникальныйИдентификатор));
			КонецЕсли;
...
Процедура ВкладкаОткрытьФайл(Файл, ТекстовыйДокумент = Неопределено)
	...
	Если Не ВозможноОткрытьФайл(Файл) Тогда
		Возврат;
	КонецЕсли;
...
Функция ВкладкаДобавитьНаСервере(Индекс)
	...
	КнопкаЗакрыть = Элементы.Добавить("Закрыть" + ВкладкаДекорация.Имя, Тип("КнопкаФормы"),
	...
	Если 0 < ПоказыватьЗначенияПеременныхПослеВыполнения Тогда
		...
		Если 0 < Переменные.length Тогда
			...
			Для Индекс = 0 По Переменные.length - 1 Цикл
				...
				Код = Код + Символы.ПС
					+ "Попытка SPd201bf6.Вставить(""" + Имя + """,Вычислить(""" + Имя + """)); Исключение КонецПопытки;"
					+ Символы.ПС;
			КонецЦикла;
...
Процедура СППрочитать(ТМППутьКФайлу)
	...
	Shell.Run("" + СПКорневойКаталог + "V8Unpack.exe -U " + """" + ?(ИзменятьРаспакованный, УжеРаспакованныйФайл.ПолноеИмя, ТМППутьКФайлу)
		+ """" + " " + СПРабочийКаталог + ИмяПодкаталога, 0, Истина);
	...
	Shell.Run("" + КаталогFileStorage + "unzip.exe -o " + СПРабочийКаталог + ИмяПодкаталога + "\PackBlock.data -d " + СПРабочийКаталог + "PackBlock", 0, Истина);
	...
	Если Файл.Размер() > 0 Тогда
		Shell.Run("" + КаталогFileStorage + "unzip.exe -o " + СПРабочийКаталог + ИмяПодкаталога + "\IndexPackBlock.data -d " + СПРабочийКаталог + "IndexPackBlock", 0, Истина);
	КонецЕсли;
...
Процедура СПВычислитьИндексКартинки(пСтрокаДерева, пКоличествоПодчиненных = Неопределено)
	...
	Если пСтрокаДерева.ЭтоПапка И Не ПустаяСтрока(пСтрокаДерева.ПутьКHTML)
		Или КоличествоПодчиненных > 0 И Не пСтрокаДерева.ЭтоПапка
	Тогда
...
Процедура СПИзвлечьИзFileStorage(пПуть)
	...
	Shell.Run(КаталогFileStorage + "unzip.exe -o "
		+ СПКаталог + СПИмяФайлаСправки + "\source\FileStorage.data" + " "
		+ """" + пПуть + """"
		+ " -d " + СПКаталог + СПИмяФайлаСправки + "\FileStorage", 0, Истина);
...
&НаКлиентеНаСервереБезКонтекста
Функция СтрЗаканчиваетсяНа(Строка, СтрокаПоиска) Экспорт
...
Функция СтрРазделить(Знач Строка, Разделитель, ВключатьПустые = Истина) Экспорт
	...
	// + Шайсламов 29.03.24 Работа как в 8.3.6 СтрРазделить()
	Если СтрДлина(Разделитель) > 1 Тогда
		Возврат _СтрРазделитьПлатформенно(Строка, Разделитель, ВключатьПустые, Результат);
	КонецЕсли;
	// -
...
&НаКлиентеНаСервереБезКонтекста
Функция _СтрРазделитьПлатформенно(Строка, Разделитель, ВключатьПустые, Результат)
	
	// Получаем массив разделителей
	ДлинаРазделителя = СтрДлина(Разделитель);
	Разделители = Новый Массив;	
	Для Инд = 1 По ДлинаРазделителя Цикл
		Разделители.Добавить(Сред(Разделитель, Инд, 1));
	КонецЦикла;
	
	// Получаем положения разделителей в строке
	ПоложенияРазделителей = Новый Массив;
	Для каждого ОдиночныйРазделитель Из Разделители Цикл
		СимволЗамены = ?(ОдиночныйРазделитель = "%", "#", "%");
		СтрокаПоиска = Строка;
		ПоложениеРазделителя = Найти(СтрокаПоиска, ОдиночныйРазделитель);
		Пока ПоложениеРазделителя > 0 Цикл
			ПоложенияРазделителей.Добавить(ПоложениеРазделителя);
			СтрокаПоиска = Сред(СтрокаПоиска, 0, ПоложениеРазделителя - 1) + СимволЗамены + Сред(СтрокаПоиска, ПоложениеРазделителя + 1);
			ПоложениеРазделителя = Найти(СтрокаПоиска, ОдиночныйРазделитель);
		КонецЦикла;
	КонецЦикла;
	
	// Сортируем по возрастанию разделители, чтобы корректно обрезать строку
	Список = Новый СписокЗначений;
	Список.ЗагрузитьЗначения(ПоложенияРазделителей);
	Список.СортироватьПоЗначению(НаправлениеСортировки.Возр);
	ПоложенияРазделителей = Список.ВыгрузитьЗначения();
	
	// Срезаем строку по принципу: (Положение предыдущего разделителя; Положение текущего разделителя)
	// Границы не включаем; если включаем пустые, то не пропускаем пустоту между разделителями
	ПредыдущееПоложение = 0;
	Для каждого ПоложениеРазделителя Из ПоложенияРазделителей Цикл
		Если Не ВключатьПустые И ПредыдущееПоложение + 1 = ПоложениеРазделителя Тогда
			ПредыдущееПоложение = ПоложениеРазделителя;
			Продолжить;
		КонецЕсли;                  
		Результат.Добавить(Сред(Строка, ПредыдущееПоложение + 1, ПоложениеРазделителя - 1 - ПредыдущееПоложение));
		ПредыдущееПоложение = ПоложениеРазделителя;
	КонецЦикла;
	
	// Проверка от последнего разделителя до конца строки
	Если СтрДлина(Строка) > ПоложениеРазделителя Или ВключатьПустые Тогда
		Результат.Добавить(Сред(Строка, ПоложениеРазделителя + 1));
	КонецЕсли;
...
Функция СтрШаблон(Шаблон,
	...
	Параметр10 = Неопределено) Экспорт
