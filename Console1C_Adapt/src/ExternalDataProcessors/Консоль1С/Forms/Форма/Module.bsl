...
Процедура УстановитьКонсольЗапросов(ОбработкаОбъект)
	...
	КонсольЗапросовДД = ОбработкаОбъект.ПолучитьМакет("КонсольЗапросов");
...
Функция _СтрРазделитьПлатформенно(Строка, Разделитель, ВключатьПустые, Результат)
	...
	Для Каждого ОдиночныйРазделитель Из Разделители Цикл
		...
		Пока ПоложениеРазделителя > 0 Цикл
			...
			СтрокаПоиска = Сред(СтрокаПоиска, 0, ПоложениеРазделителя - 1) + СимволЗамены + Сред(СтрокаПоиска, ПоложениеРазделителя
																											   + 1);
			ПоложениеРазделителя = Найти(СтрокаПоиска, ОдиночныйРазделитель);
		КонецЦикла;
	КонецЦикла;
