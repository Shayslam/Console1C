...
////////////////////////////////////////////////////////////////////////////////
// Обработчики событий формы и элементов шапки формы
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Параметры.Свойство("ТекстПриОткрытии") Тогда
...
////////////////////////////////////////////////////////////////////////////////
// Обработчики событий элементов таблиц
...
Процедура ФайлыДеревоПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВопросаОбУдаленииВкладки", ЭтотОбъект);
...
#Область ОбработчикиСобытийЭлементовТаблицыСПДерево
...
////////////////////////////////////////////////////////////////////////////////
// Обработчики команд формы и служебные
...
Процедура ПереходПереоткрытьОбработку(Команда)
	...
	Если Не ПустаяСтрока(ИмяЭтойОбработки) Тогда
		ЗакрытьКонсоль();
		ОткрытьФорму("ВнешняяОбработка." + ИмяЭтойОбработки + ".Форма");
	Иначе
...
Процедура СППоискПоСправке(Команда)
	...
	Если Элементы.СтраницыПравые.Видимость = Ложь
		...
		СПОткрыть(ЭтотОбъект.Команды.Найти("СПОткрыть"));
	КонецЕсли;
...
Процедура ВыполнитьКодЧерезОбработку(Конфигурация, Текст, ТекстМодуляОбъекта)
	...
	//АдресПеременных = НайтиПеременные(Текст, ТекстМодуляОбъекта);
...
Процедура ВыполнитьКодЧерезОбработкуНаКлиенте(Конфигурация, ИмяОбработки, МетодВхода, СтрокаПараметров)
	...
	Если Конфигурация.Свойство("ОткрытиеОбработки") И Конфигурация.ОткрытиеОбработки = Истина Тогда
		...
		КонецЕсли;
	Иначе
		Модуль = ПолучитьФорму("ВнешняяОбработка." + ИмяОбработки + ".Форма");
		Выполнить("Модуль." + МетодВхода + "(" + СтрокаПараметров + ")");
	КонецЕсли;
...
&НаКлиенте
Функция СформироватьОбработку(Конфигурация, МодульОбъекта = "", МодульФормы = "")
	...
	ИмяОбработки = ПодключитьШаблонОбработки(ИмяГотовойОбработки);
...
&НаКлиенте
Функция ПодключитьШаблонОбработки(ИмяШаблонаОбработки)
	...
	Возврат ПодключитьОбработку(ИмяШаблонаОбработки, АдресШаблонаОбработки);
...
&НаКлиенте
Функция ВыгрузитьШаблон(ИмяШаблонаОбработки, ИмяКаталогаШаблонаОбработки, Конфигурация)
...
Функция СправкаОбОбработке()
	...
	ОбОбработке = ЭтотОбъект().СведенияОВнешнейОбработке();
...
Процедура ФайлыДеревоОбновить(Знач ФайлКаталог, СохранитьРазвертку = Ложь)
	...
	Для Каждого Файл Из МассивФайлов Цикл
		СтруктураФайла = Новый Структура("ЭтоФайл, Имя, ИмяБезРасширения, ПолноеИмя, Путь, Расширение", Файл.Этофайл());
...
Функция ФайлыДеревоПеретаскиваниеСтроки(Знач Приемник, ПриемникРодитель, Знач ТекущиеДанные, СтруктураВозврата)
	...
	Если Приемник.ЭтоФайл Тогда
		...
		Если ПеретаскиваниеНиже Тогда
			...
			Для Каждого СтрокаПриемника Из ПриемникРодитель.ПолучитьЭлементы() Цикл
				Если Не СтрокаПриемника.ЭтоФайл И СтрНайти(ТекущиеДанные.Путь, СтрокаПриемника.ПолноеИмя) > 0 Тогда
					Строка = СтрокаПриемника.ПолучитьИдентификатор();
		...
		Иначе
			Строка = ПриемникРодитель.ПолучитьИдентификатор();
...
&НаКлиенте
Функция ФайлыДеревоРазвертка(СоответствиеРазвертки)
...
Функция ФайлыТекстЗапускаПоУмолчанию()
	...
	|Конфигурация.Вставить(""Тип"", ""bsl"");
	|Конфигурация.Вставить(""Контекст"", ""НаСервере"");
...
Функция ВкладкаЗакрыть(Вкладка = Неопределено, БезВопросов = Ложь, Открывать = Истина)
	...
	Если Не БезВопросов И Вкладка.Модифицирована Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеОтветаОНесохраненнойВкладке", ЭтотОбъект, Вкладка);
...
Процедура СППрочитать(ТМППутьКФайлу)
	...
	// Определяем имя файла hbk, с которым будем работать - это зависит от команды и от уже проделанной работы с файлами
	Если ИзменятьРаспакованный Тогда
		ИмяФайла = УжеРаспакованныйФайл.ПолноеИмя;
	Иначе
		ИмяФайла = ТМППутьКФайлу;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПодключитьОбработку(ПолноеИмя, АдресРасширения)
	
	Попытка
		Обработка = ВнешниеОбработки.Создать(ПолноеИмя, Ложь, Защита());
		ИмяОбработки = ВнешниеОбработки.Подключить(АдресРасширения, , Ложь, Защита());
	Исключение
		ИмяОбработки = ВнешниеОбработки.Подключить(АдресРасширения, , Ложь, Защита());
	КонецПопытки;
